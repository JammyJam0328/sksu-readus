            ...
            <ol class="mt-2 space-y-3 text-sm">
                @foreach ($upcomingEvents as $key => $event)
                    <li wire:key="{{ $key }}-{{ $event->title }}"
                        ...
                        <div class="flex-auto">
                            <h3 class="font-semibold text-gray-700 underline cursor-pointer ">
                                {{ $event->title }}</h3>
                            <dl class="flex flex-col mt-2 text-gray-500 xl:flex-row">
                                <div class="flex items-start space-x-3">
                                    ...
                                    <dd class="text-gray-600">
                                        <time
                                            datetime="2022-01-10T17:00">{{ date('M d, Y H:s A', strtotime($event->start_date)) }}</time>
                                    </dd>
                                </div>
                                <div
                                    class="mt-2 flex items-start space-x-1 xl:mt-0 xl:ml-3.5 xl:border-l  xl:border-opacity-50 xl:pl-3.5">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <x-shared.my-countdown :expires="$event->start_date" />
                                </div>
                            </dl>
                        </div>
